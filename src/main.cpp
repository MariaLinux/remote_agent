...
int main(int argc, char *argv[]) {
  ...
  remote_agent::Publisher<std::string> publisher(endpoint, "topic");
  publisher.connect();
  ...
  remote_agent::Publisher<std::string> publisher2(endpoint, "topic_2");
  publisher2.connect();
  std::thread t1 = std::thread([&]() {
    while (true) {
      std::this_thread::sleep_for(std::chrono::seconds(1));
      publisher2.publish("Topic World 2");
    }
  });
  remote_agent::Subscriber<std::string> subscriber("tcp://localhost:5555",
                                                   "topic");
  subscriber.connect();
  subscriber.subscribe(
      [&](const std::string &message) { std::cout << message << std::endl; });
  remote_agent::Subscriber<std::string> subscriber2("tcp://localhost:5555",
                                                    "topic_2");
  subscriber2.connect();
  ...
  std::cout << getCurrentTimeStr() << " - Starting one-shot timer for 2 seconds"
            << std::endl;
  ...
  std::cout << getCurrentTimeStr()
            << " - Starting periodic timer with 1 second interval" << std::endl;
  ...
  std::cout << getCurrentTimeStr() << " - Starting timer with parameters"
            << std::endl;
  int counter = 0;
  paramTimer.startPeriodic(
      500,
      [&counter](const std::string &message) {
        counter++;
        std::cout << getCurrentTimeStr() << " - " << message
                  << " (count: " << counter << ")" << std::endl;
        if (counter >= 6) {
          std::cout << getCurrentTimeStr() << " - Reached target count"
                    << std::endl;
        }
      },
  ...
  auto task = runner.parseTasks(
      "/home/amin/code/amin/marialinux/remote_agent/build/Debug/task1.yaml");
  ...
  for (const auto &step : task.steps) {
    ...
    for (const auto &env : step.environments) {
      std::cout << "Environment: " << env.first << "=" << env.second
                << std::endl;
    }
