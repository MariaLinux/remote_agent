...
namespace remote_agent {
    ...
    std::optional<AccountConfig> Config::getAccountByName(const std::string& name) const {
            ...
            if (config["accounts"]) {
                for (const auto& account_node : config["accounts"]) {
                    ...
                    if (account_node["protocol"]["smtp"]) {
                        ...
                        account.smtp.auth_method = smtp["auth_method"].as<std::string>("login");
                    ...
                    if (account_node["protocol"]["imap"]) {
                        ...
                        account.imap.auth_method = imap["auth_method"].as<std::string>("login");
                    ...
                    if (account_node["credentials"]) {
                        ...
                        if (account.credentials.password.substr(0, 2) == "${" && 
                            ...
                            if (env_val) {
                                ...
                                syslog(LOG_WARNING, "Config/loadConfig: Environment variable %s not found for account %s", 
                                    env_var.c_str(), account.name.c_str());
                            }
                        }
                    }
                    
                    account.default_folder = account_node["default_folder"].as<std::string>("inbox");
    ...
    std::optional<ImapFilterConfig> Config::parseImapFilter(const YAML::Node& imap_node) {
        if (!imap_node["filter"]) {
            return std::nullopt;
        }
    ...
    mailio::imap::search_condition_t Config::parseCondition(const YAML::Node& condition_node) {
        std::string type = condition_node["type"].as<std::string>();
        mailio::imap::search_condition_t::key_type key;
        
        if (type == "ALL") key = mailio::imap::search_condition_t::ALL;
        else if (type == "SID_LIST") key = mailio::imap::search_condition_t::SID_LIST;
        else if (type == "UID_LIST") key = mailio::imap::search_condition_t::UID_LIST;
        else if (type == "SUBJECT") key = mailio::imap::search_condition_t::SUBJECT;
        else if (type == "BODY") key = mailio::imap::search_condition_t::BODY;
        else if (type == "FROM") key = mailio::imap::search_condition_t::FROM;
        else if (type == "TO") key = mailio::imap::search_condition_t::TO;
        else if (type == "BEFORE_DATE") key = mailio::imap::search_condition_t::BEFORE_DATE;
        else if (type == "ON_DATE") key = mailio::imap::search_condition_t::ON_DATE;
        else if (type == "SINCE_DATE") key = mailio::imap::search_condition_t::SINCE_DATE;
        else if (type == "NEW") key = mailio::imap::search_condition_t::NEW;
        else if (type == "RECENT") key = mailio::imap::search_condition_t::RECENT;
        else if (type == "SEEN") key = mailio::imap::search_condition_t::SEEN;
        else if (type == "UNSEEN") key = mailio::imap::search_condition_t::UNSEEN;
        else syslog(LOG_ERR, "Unknown search condition type: %s" , type.c_str());
        ...
        else if (key == mailio::imap::search_condition_t::SID_LIST || key == mailio::imap::search_condition_t::UID_LIST) {
            // Parse ID list
            if (condition_node["value"]) {
                std::list<mailio::imap::messages_range_t> ranges;
                for (const auto& range_node : condition_node["value"]) {
                    if (range_node["range"]) {
                        auto range_list = range_node["range"].as<std::vector<unsigned long>>();
                        if (range_list.size() == 1) {
                            // Single ID
                            ranges.push_back(std::make_pair(range_list[0], std::optional<unsigned long>{}));
    ...
    std::optional<SmtpConfig> Config::parseSmtpConfig(const YAML::Node& smtp_node) {
        ...
        bool filled;
        if (smtp_node["from"]) {
            smtp_config.from = mailio::mail_address(mailio::string_t(smtp_node["from"]["name"].as<std::string>(), 
                                                    "UTF-8", mailio::codec::codec_t::UTF8), 
    ...
    mailio::mailboxes Config::parseMailboxes(const YAML::Node& mail_node) {
        ...
        for (const auto& item_node : mail_node) {
            ...
            if (item_node["address"]) {
                ...
                addr = mailio::mail_address(mailio::string_t(item_node["name"].as<std::string>(), 
                                        "UTF-8", mailio::codec::codec_t::UTF8), 
                ...
                if (item_node["list"].IsSequence()) {
                    for (const auto& member_node : item_node["list"]) {
                        ...
                        member = mailio::mail_address(mailio::string_t(member_node["name"].as<std::string>(), 
                                                                            "UTF-8", mailio::codec::codec_t::UTF8), 
